...
| **🗺️ Bloco 2** | 12 min | Ilha → Floresta → Rio | `NavigationLink` & Pop to root |
| **🐻 Bloco 3** | 18 min | Fuga do Urso & Caminho Alternativo | `NavigationPath` programático, Pop simples & Back Button Oculto |
...
*   **Seu objetivo:** Substituir botões estáticos por links de navegação. Além disso, ao chegar ao Rio, a opção de "Explorar mais a ilha" deve resetar o caminho de volta para o início.
*   **Onde mexer:**
    *   `ContentView.swift`
...
*   **Dica:** Troque `Button` por `NavigationLink`. Para resetar o caminho, zere a lista de navegação (`path`).

### 🐻 Bloco 3 — Urso e volta na ilha (18 min)
*   **Sintoma:** Fugir do urso ou tentar dar a volta na ilha não faz nada. Além disso, o botão de retorno nativo da barra superior sumiu!
*   **Seu objetivo:**
    1.  Fazer com que "Fugir Correndo" retorne exatamente 1 tela no histórico de navegação.
    2.  Fazer com que "Dar a volta na ilha" adicione programaticamente uma nova ilha no histórico (navegação baseada em dados).
    3.  A ação de "Voltar ao Início" deve resetar a rota de volta para a praia principal.
*   **Onde mexer:** [CriaturaView.swift](treasurehunt/GameViews/CriaturaView.swift) (`// TODO: Bloco 3`)
*   **Dica:** Manipule a variável `path` que controla a pilha de navegação. Use `.removeLast()` para voltar e `.append()` para avançar de forma programática. O botão de retorno foi ocultado propositalmente com `.navigationBarBackButtonHidden(true)`.
